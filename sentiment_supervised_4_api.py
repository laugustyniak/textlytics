...
def get_dataset_with_kfolds_indexes(base_path, output_folder, dataset_filter,
	...
	for dataset in datasets:
		...
		if stars is not None:
			...
			m = df['review/score'].value_counts().min()
			t = pd.DataFrame()
			for i in stars:
				t = t.append(df[df['review/score'] == i].head(m))
			df = t
		...
		for n_gram_name, n_grams_range in features_ngrams.iteritems():
			for clf_name, classifier in classifiers.iteritems():
				...
				joblib.dump(pipeline,
				            join(output_folder, 'Pipeline-{}-{}-{}-stars-{}-{}.pkl'.format(clf_name, vectorizer_type, n_gram_name, '-'.join([str(s) for s in stars]), dataset_name)),
				            compress=9)
